...
class CardManagerApp(tk.Tk):
    def __init__(self, manager):
        ...
        # カード名リストボックス
        self.card_name_listbox = tk.Listbox(main_frame, font=("Helvetica", 12), width=30, height=10)
        self.card_name_listbox.grid(row=1, column=0, pady=10)
        
        # 枚数リストボックス
        self.card_count_listbox = tk.Listbox(main_frame, font=("Helvetica", 12), width=10, height=10)
        self.card_count_listbox.grid(row=1, column=1, pady=10)

        self.update_listbox()  # リストボックスにカードを表示

        # リストボックスのクリックイベントをバインド
        self.card_name_listbox.bind('<<ListboxSelect>>', self.on_card_select)
    ...
    def update_listbox(self):
        self.card_name_listbox.delete(0, tk.END)  # カード名リストボックスの内容をクリア
        self.card_count_listbox.delete(0, tk.END)  # 枚数リストボックスの内容をクリア
        cards = self.manager.get_cards()
        for card, count in cards.items():
            self.card_name_listbox.insert(tk.END, card)
            self.card_count_listbox.insert(tk.END, f"{count}枚")
    ...
    def search_card(self):
        ...
        self.card_name_listbox.delete(0, tk.END)  # 検索結果を表示するためリストボックスをクリア
        self.card_count_listbox.delete(0, tk.END)  # 枚数リストボックスもクリア
        if results:
            for card, count in results.items():
                self.card_name_listbox.insert(tk.END, card)
                self.card_count_listbox.insert(tk.END, f"{count}枚")
        else:
            self.card_name_listbox.insert(tk.END, f"{card_name}を含むカードは見つかりませんでした。")
            self.card_count_listbox.insert(tk.END, "")

    # カード名が選択された時の処理
    def on_card_select(self, event):
        # 選択されたカードのインデックスを取得
        selected_index = self.card_name_listbox.curselection()
        if selected_index:
            # 選択されたカード名を取得
            card_name = self.card_name_listbox.get(selected_index)
            self.card_name_entry.delete(0, tk.END)  # カード名エントリーをクリア
            self.card_name_entry.insert(0, card_name)  # 選択されたカード名をエントリーにセット

            # 対応する枚数を表示するため、枚数リストボックスからも取得
            count_text = self.card_count_listbox.get(selected_index)
            self.card_count_entry.delete(0, tk.END)  # 枚数エントリーをクリア
            self.card_count_entry.insert(0, count_text.replace('枚', '').strip())  # 枚数をエントリーにセット
